//-----------------------------------------------------------------------
//
//  This is a box for the luchtSensor
//
...
include <./YAPPgenerator_v3.0.2.scad>


myPcb = "ESP32-DevkitC32D.stl";

if (false)
{
  translate([57.5, 2.5 , 9]) 
  {
    rotate([0,0,90]) color("gray") import(myPcb);
  }
}
...
pcbLength           = 56;   //-- front to back (X axis)
pcbWidth            = 28.0; //-- side to side (Y axis)
pcbThickness        =  1.7; 
standoffHeight      =  9; //-- How much the PCB needs to be raised from the base to leave room for solderings and whatnot
standoffDiameter    =  4;
...
[
  // Default Main PCB - DO NOT REMOVE the "Main" line.
   ...
   ,["PMS5003",     50, 38, 0, 38, 21.5, 0, 0, 0, 0.4]
];
...
// --Preview --
previewQuality            = 3;          //-> from 1 to 32, Default = 5
...
showPCB                   = false;      //-> Show the switches (for pushbuttons) : only in preview 
...
printMessages             = true;       //-> Print messages during generation
//---------------------------
//--     C O N T R O L     --
//---------------------------
...
pcbStands = 
[
  [3.3, 2.0, yappBoth, yappLidOnly, yappBackLeft, yappBackRight],
  [1.5, 2.2, yappBoth, yappLidOnly, yappFrontLeft, yappFrontRight]
];
...
//-              0,                  1,                2, 3,   4,   5,   6,    7, -rest-
  [(shellLength/2), (shellWidth/2)-6.5, standoffHeight+5, 2.5, 4.5, 3.5, 10, default, yappCoordBoxInside]
];
...
//  Parameters:
//   Required:
//    p(0) = from Back  //-- from left!
//    p(1) = from Left  //-- height
//    p(2) = width
//    p(3) = length
//    p(4) = radius
...
[
  //maskHoneycomb
 //   [ shellLength/2, 12, 27, 21, 2, yappRoundedRect, maskCustom1, yappCenter] //-- Custom Mask defined above.
   [ 49.8, pcbWidth-7.3, 1, 1, 1.4, yappCircle] //-- Enable Button.

];

cutoutsFront =  
[
//[13, 0, 10.5, 7, 1, yappRoundedRect, yappCenter, yappCoordPCB] //-- USB.
  [15, 3, 14.5, 8, 1, yappRoundedRect, yappCenter, yappCoordPCB] //-- USB.
];
...
[
//   0,   1,  2,  3,  4,        5        6 
  [ 15.5, 0, 20, 20, 10, yappCircle, yappCenter] //-- gewoon een circel.
 ,[ 40,   8, 20,  4, 1.7, yappRoundedRect, yappCenter] 
];
...
snapJoins   =   
[
   [20, 5, yappBack, yappCenter, yappSymmetric]
  ,[64, 5, yappFront, yappCenter]
  ,[34, 5, yappFront, yappCenter]
  ,[15, 5, yappLeft, yappCenter, yappSymmetric]
  ,[30, 2, yappRight, yappCenter] //, yappSymmetric]
];
...
//--  [ 50, pcbWidth-8.5, 1, 1, 2, yappCircle] //-- Enable Button.
  translate([52.2, 23.1, -7]) color("red") 
  difference()
  {
    cylinder(h=7, d=4.5, $fn=30);
    cylinder(h=7, d=2.5, $fn=30);
  }
} // hookLidInside()
...
{
  translate([shellInsideLength-5.9, 70, -1])  cube([8, 2, 14]);
  translate([-6.2, 70, -1])                    cube([8, 2, 14]);
  translate([shellInsideLength-5.9, 50, -1])  cube([8, 2, 14]);
  translate([-6.2, 50, -1])                    cube([8, 2, 14]);
...
YAPPgenerate();
// translate([3.5,71,10]) color("black") rotate([0,90,0]) cube ([1,2,50]);